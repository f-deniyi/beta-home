...
const PromotionManagement = () => {
    ...
    const { data: userData, isError, error, isLoading: fetchingDetails, isSuccess } = useGetUserDetailsManager();
    ...
    return (
        <BaseDashboardNavigation title={"Promotion Management"} hideSearch={true}>
            {
                isLoading || fetchingDetails ? <Loader /> : <>
                    <div className="mb-5">
                        ...
                        <div className="bg-white rounded-lg py-[38px] px-[23px] ">
                            <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-3 xxl:grid-cols-3 gap-5">
                                {banners?.map((item, index) => (
                                    <BannerCard type={index} banner={item} setBannerId={setBannerId} />
                                ))}
                            </div>
