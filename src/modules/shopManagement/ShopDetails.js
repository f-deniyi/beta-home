...
import { locationIcon, verifiedIcon, product, services } from "../../assets/icons";
...
import UseSuspendShop from "./controllers/suspend_shop";
...
const ShopDetails = () => {
    ...
    const { products, pagination, isLoading: fetchingShopProducts } = useGetShopsProductsQuery({
        enabled: Boolean(shopId),
        shopId,
        page: activePage,
    });
    ...
    const { verifyShop, isLoading: verifying } = VerifyShopMutation({ shopId })
    // const { suspendShop, isLoading: suspending } = SuspendShopMutation(
    //     {
    //         shopId,
    //         action
    //     })

    const { isSuccess, isLoading: suspending, refetch: suspendShop } = UseSuspendShop({
        shopId,
        action: action,
            ...
            {isLoading || fetching || fetchingShopProducts ? (
                ...
                <>
                    <div class="relative w-full">
                        <div class="bg-cover bg-center h-[207px]  rounded-bl-[10px] rounded-br-[10px] w-full shadow-lg" style={{ backgroundImage: `url(${shop?.cover_image?.original})` }}></div>
                        <div class="absolute bottom-[0px] left-[80px] transform -translate-x-1/2 translate-y-1/2 rounded-full overflow-hidden border-4 border-brandPrimary">
                            <img class="object-cover h-[120px] w-[120px] " src={shop?.logo?.original} alt="Profile Picture" />
                        </div>
                        <div class="absolute top-[10px] right-[20px] flex items-center">
                            ...
                            <CustomButton
                                ...
                                isLoading={verifying}
                                disabled={verifying}
                                className={!shop?.isVerified ? "rounded-full bg-green-600 text-white shadow-xl flex items-center justify-center px-6 py-2 !hover:bg-green-700" : "rounded-full bg-rose-700 text-white shadow-xl  mr-[5px] flex items-center justify-center px-6 py-2"}
                                onClick={(e) => {
                                    console.log(e)
                                    verifyShop({
                            ...
                            <CustomButton
                                ...
                                disabled={suspending}
                                className={"rounded-full bg-slate-700 text-white shadow-lg  flex items-center justify-center px-6 py-2"}
                                onClick={(e) => {
                                    // console.log(e)
                                    setAction(!shop?.isSuspended ? "suspend" : "unsuspend")
                                    suspendShop()
                                }}
                            />
                        </div>
                    ...
                    <div class="ml-40 flex items-center justify-between">
                        <div className="mt-3 ">
                            <div className="flex items-center ">
                                <h3 className="text-[21.457px] text-black font-medium mr-2">{shop?.name}</h3>
                                {shop?.isVerified && <img src={verifiedIcon} className="object-cover h-[15px] w-[15px] p-[0.7px] rounded-full " />}

                            </div>
                            <div className="flex items-center mb-2">
                                <img src={locationIcon} className="object-cover  p-[0.7px] rounded-full mr-1" />
                                <p className="text-[15.735px] text-[#696969] font-normal mr-2">{formatAddress(shop?.address)}</p>
                            </div>
                            <div className="flex items-center ">
                                <p className="bg-brandPrimary border-1 border-black rounded-full font-medium px-2 py-1 text-[11.44px] me-2">Created</p>
                                <p className="font-light text-[11.444px] text-[#69696]">{`${formatMonth(shop?.createdAt)} ago`}</p>
                            </div>
                        </div>
                        <div class="">
                            <div className="flex">
                                <button className="rounded-full bg-brandPrimary  mr-[10px] flex items-center justify-center px-5 py-4"
                                    onClick={() => document.getElementById("product_orders").showModal()}
                                >
                                    <img src={product} alt='icon' className="w-[14px] h-[14px] mr-2" />
                                    <p className="text-[13px] font-medium">View Product Order</p>
                                </button>
                                <div className="divider divider-horizontal w-[0.3px] bg-[#A5A5A5]" />
                                <button className="rounded-full bg-brandPrimary  mr-[10px] flex items-center justify-center px-5 py-4"
                                    onClick={() => document.getElementById("services_request").showModal()}
                                >
                                    <img src={services} alt='icon' className="w-[14px] h-[14px] mr-2" />
                    ...
                    <div className="w-full mt-8">
                        <div className="tabs w-full flex">
                            <a className={`tab tab-bordered  ${activeTab === 1 ? 'tab-active border-brandPrimary' : ""} w-1/2 text-[15px] font-semibold`}
                                onClick={() => {
                                    setActiveTab(1)
                                }}
                            >Products</a>
                            <a className={`tab tab-bordered  ${activeTab === 2 ? 'tab-active' : ""} w-1/2 text-[15px] font-semibold`}
                                onClick={() => {
                                    setActiveTab(2)
                                }}
                            >Services</a>
