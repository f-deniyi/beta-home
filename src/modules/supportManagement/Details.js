...
const SupportDetails = () => {
    const { data, isLoading } = useGetUserDetailsManager();
    ...
    const { support_details, isLoading: fetchingSupport } = useGetSupportDetailsQuery({
        enabled: Boolean(ticketId),
        ticketId: ticketId
    })
    const { messages, isLoading: fetchingMessages } = useGetSupportMessageQuery({
        enabled: Boolean(ticketId),
        ticketId: ticketId
    })
    ...
    function getFileType(fileUrl) {
        ...
        const imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp']; // Add more image extensions as needed
    ...
    return (
        <BaseDashboardNavigation title={'Support Management'}>
            {
                isLoading || fetchingSupport || fetchingMessages ? <Loader /> : <>
                    ...
                    <div className='bg-white p-[27px] rounded-[10px] '>
                        ...
                        <div className='mt-5 flex flex-col'>
                            {
                                messages?.map(el =>
                                    <div className={`mb-2 ${el?.sender?._id === userId ?
                                        'self-end text-right' :
                                        ''
                                        }`}>
                                        {el?.attachments.length > 0 && <div className='bg-gray-200 flex flex-wrap gap-2 max-w-[230px] p-2 mb-2 rounded-[5px]'>
                                            {
                                                el?.attachments?.map(el =>
                                                    <div
                                                    ...
                                                    >
                                                        {getFileType(el) === 'image' ?
                                                            <img
                                                                src={el}
                                                                className='rounded-[5px] object-cover h-[100px] w-full  cursor-pointer'
                                                                alt="Attachment"
                                                            /> : <div
                                                                className='cursor-pointer rounded-[5px] flex items-center justify-center text-gray-800 bg-zinc-500 font-medium'
                                                            >{getFileType(el)}</div>}
                                                    </div>
                                                )
                                            }
                                        </div>}
                                        <div key={el?._id} className={`p-2 rounded-lg mb-1 text-left max-w-[350px] shadow-md ${el?.sender?._id === userId ?
