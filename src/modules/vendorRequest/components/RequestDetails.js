...
const RequestDetails = ({ user }) => {
  const { updateVendorRequests, isLoading, isSuccess } =
    UpdateVendorRequestsMutation();
  ...
  return (
    <>
      <ModalManagement id={"vendor_request"} hideCancel={true}>
        <div className="flex items-center justify-between mb-6">
          <p className="text-[18px] font-medium ">Customer Details</p>
          <div
            onClick={() => document.getElementById("vendor_request").close()}
            role="button"
          >
        ...
        {user?.status === "pending" && (
          <div className="flex gap-x-2">
            <CustomButton
              buttonText={"Accept"}
              isLoading={isLoading && isApproval}
              disabled={isLoading}
              onClick={() => {
                setIsApproval(true);
